...
2. Поднимите инфраструктуру:
   ...
   docker-compose up -d
   ```
   Контейнер `db` поднимет PostgreSQL 15. При старте контейнера `web` автоматически выполнится `alembic upgrade head`, чтобы база всегда была в актуальном состоянии.
...
4. Запустите сервис:
   ...
   ```
   Для применения миграций вручную (например, в поднятом контейнере) используйте:
   ```bash
...
- В проекте **нельзя** создавать файлы или директории с именем `sqlalchemy` — они перекроют библиотечный пакет SQLAlchemy и приведут к ошибкам импорта.
- При запуске контейнера `web` миграции применяются автоматически через `alembic upgrade head`. Локальный запуск через `uvicorn` остаётся возможным без изменений.
