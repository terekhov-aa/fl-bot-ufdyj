...
2. Поднимите инфраструктуру (миграции применяются автоматически при старте `web`):
   ```bash
   docker-compose up -d
   ```
   Контейнер `db` поднимет PostgreSQL 15, а при запуске `web` автоматически выполнится `alembic upgrade head`.
3. Установите зависимости (для локального запуска вне Docker):
   ```bash
   pip install -r requirements.txt
   ```
4. Запустите сервис локально (если нужно без Docker):
   ```bash
...
- При старте контейнера `web` автоматически выполняется `alembic upgrade head`, поэтому схема БД всегда актуальна.
...
- Не создавайте в проекте файлы или директории с именем `sqlalchemy`, чтобы не конфликтовать с одноимённой библиотекой.
- Все shell-скрипты хранятся с переводами строк LF (`.gitattributes` следит за этим); CRLF приведёт к ошибкам вида `env: 'bash\r': No such file or directory`.
