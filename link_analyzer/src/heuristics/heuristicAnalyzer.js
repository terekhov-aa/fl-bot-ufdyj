...
/**
 * @param {import('../models/ParsedPage') | null} parsed
...
function decideAnalysisMode(parsed, signals) {
  ...
  const hasEnoughText = textLength >= 800;
  ...
  const hugeHtml = htmlLength > 150_000;
  ...
  if (hasEnoughText && !likelyViewerLike) {
    reasons.push('ample_text_present');
  }
